...
use Softworx\RocXolid\DevKit\Console\Commands\Generate\Traits\Configurable;
...
class SyntaxBuilder
{
    ...
    private function createSchemaForUpMethod($schema, $meta)
    {
        ...
        throw new RuntimeException(sprintf("Could not determine what you are trying to do.\nMeta: %s", print_r($meta, true)));
    }
    ...
    {
        return file_get_contents(sprintf('%s/%s', $this->getConfig('stub_directory'), $this->getConfig('schema_create_stub')));
    }
    ...
    {
        return file_get_contents(sprintf('%s/%s', $this->getConfig('stub_directory'), $this->getConfig('schema_change_stub')));
    }
